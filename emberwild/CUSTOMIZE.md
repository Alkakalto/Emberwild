...
| `data.js` | Creatures, stats, type advantages, NPCs, teams, region names |
| `core.js` | Map terrain, combat rules, leveling, save validation |
...
Region definitions in `data.js` set NPC positions, wild species and levels, and exits. Exit `to` points to another region ID; `tx` and `ty` set the arrival tile. Optional `requires` gates an exit behind a crest. The `route` number gives the ten expedition regions their generated obstacle patterns and milestone markers. Keep arrival tiles walkable and all story characters reachable. Update the scenery in `art.js` if you move buildings, bridges or the beacon.
...
More quest chains, dialogue portraits, additional sound design, accessibility options for the map, persistent server accounts and player trading are useful next steps. This version has online rooms but no cloud story saves, controller support, emulator compatibility or installable mobile package.
